...
declare -r router="index.html"
# setting webserver
declare -r HOSTNAME="AWS"
declare -r webserver="nginx"
declare -r domain="localhost"
declare -r node="16.16.0"
# setting pgsql
declare -r pgsql="15"
...
echo
mkdir /home/$user/web
mkdir /home/$user/web/public
mkdir $serverRoot/frontend/
mkdir $serverRoot/frontend/express/
chmod -R 755 $serverRoot/
echo
echo
echo
echo -e "User\tLogin\tPassword\tDatabase" >> /home/$user/account_data.csv
echo
clear
echo
echo
...
cat << EOF > /etc/nginx/conf.d/site-nodejs-$domain.conf
server{
    listen 80;
    server_name $domain;
...
clear
cat << EOF > $serverRoot/frontend/express/$router
<!DOCTYPE html>
<html lang="ru" >
<head>
  <meta charset="UTF-8">
  <title>Test Server NodeJS</title>
...
<body>
<div class='box'>
        <span><center><h2><p>Server NodeJS configured successfully!</p></h2></br></span>
        <div id='hostname'>...</div></center>
</div>
...
<!--               <polygon id="arrow2" points="395.54 482.2 393.42 484.32 375.21 466.11 357 484.32 354.88 482.2 375.21 461.87 395.54 482.2"/>-->
                <polygon id="arrow2"/>
                <text id="text2"></text>
            </g>
            <polygon id="h3-5"
    ...
    <g id="scene1">
        <g id="hills1">
            ...
            <g id="info">
                ...
                <!--<text id="text" transform="matrix(1 0 0 1 333 360)" fill="#fff" stroke="none" stroke-miterlimit="10" stroke-width="0.8" font-size="12px" font-family="SFProDisplay-Black";>СКРОЛЛ ВНИЗ</text>-->
                <text id="text"></text>
            </g>
...
EOF
cat << EOF > $serverRoot/frontend/express/script.js
...
EOF
cat << EOF > $serverRoot/frontend/express/style.css
body,
html{
      margin: 0;
      padding: 0;
...
EOF
cat << EOF > $serverRoot/frontend/server.js
...
EOF
cat << EOF > $serverRoot/frontend/package.json
{
  "name": "server",
  "version": "0.0.1",
  "description": "Node Server template",
  "scripts": {
...
cat << EOF > /home/$user/node_install.sh
#!/bin/bash

################CONFIG###################
declare -r node="$node"
declare -r user="$user"
declare -r serverRoot="$serverRoot"
#########################################
echo
echo
echo
echo  -e " \033[32m            ***** Install NodeJS PM2 *****                  \033[0m"
sleep 5
cd /home/\$user/
...
echo
echo
echo -e "\033[32m            *****  Start NodeJS server  *****               \033[0m"
sleep 3
pm2 start \$serverRoot/frontend/server.js
clear
echo
echo
echo
echo -e "\033[32m            *****  Save for boot NodeJS server  *****               \033[0m"
pm2 save
sudo env PATH=\$PATH:/home/\$user/.nvm/versions/node/v\$node/bin /home/\$user/.nvm/versions/node/v\$node/lib/node_modules/pm2/bin/pm2 startup systemd -u \$user --hp /home/\$user
clear
echo
echo
echo
...
sleep 5
rm -f /home/\$user/aws_install_software_web_server_node_js.sh
rm -f /home/\$user/node_install.sh
sudo reboot
EOF
...
chmod 755 /home/$user/node_install.sh
systemctl start nginx
clear
/bin/su -s /bin/bash -c "/home/$user/node_install.sh" $user
